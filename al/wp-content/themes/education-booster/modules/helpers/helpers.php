...
if( !function_exists( 'educationbooster_enqueue' ) ):
/**
...
* @return void
*/
function educationbooster_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'educationbooster_scripts' , $scripts );
...
if( ! function_exists( 'educationbooster_get_related_posts' ) ):
/**
* Get related posts
*
* @since Education Booster 1.0.0
...
* 
*/
function educationbooster_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function educationbooster_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'educationbooster_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'educationbooster_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function educationbooster_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'education-booster' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'educationbooster_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function educationbooster_get_youtube_id( $url ) {
...
if( ! function_exists( 'educationbooster_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Education Booster 1.2
* @uses educationbooster_sort_category()
* @return array
*/
function educationbooster_get_the_category( $id = false ){
    $failed = true;
    ...
    if( $failed ){
      ...
      usort( $cat, 'educationbooster_sort_category' );  
    }
...
if( ! function_exists( 'educationbooster_sort_category' ) ):
/**
* Helper function for educationbooster_get_the_category()
*
* @since Education Booster 1.0.0
*/
function educationbooster_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'educationbooster_explode_string_to_int' ) ):
/**
...
*/
function educationbooster_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'educationbooster_get_menu' ) ):
/**
* Returns wp nav
*
* @since Education Booster 1.0.0
...
* @return String
*/
function educationbooster_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'educationbooster_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'educationbooster_'.$location.'_menu', $args ) );
        break;
...
if( !function_exists( 'educationbooster_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Education Booster 1.0.0
*/
function educationbooster_hex2rgba($color, $opacity = false) {
